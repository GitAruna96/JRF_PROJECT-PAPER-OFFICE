...
csv_path = r'C:\\Users\\Admin\\Aruna_jrf_projects\\Datasets\\Office-31\\paper_experiments\\results_multi_source\\multi_source_swin_tiny_patch4_window7_224_Aw_d_sac_rev_pred_aw_d_sac_predrev_results_20250916_172623.csv'
df = pd.read_csv(csv_path)

# Create output directory for plots
plot_dir = './results/sac_multiplots'
...
plt.axhline(y=82.10, color='red', linestyle='--', linewidth=2, label='Baseline (82.10)')
plt.axhline(y=best_acc, color='green', linestyle='--', linewidth=2, label=f'Best SAC ({best_acc:.1f}%)')

plt.xlabel('Epoch', fontsize=14, fontweight='bold')
plt.ylabel('Accuracy (%)', fontsize=14, fontweight='bold')
plt.title('SAC Performance: Multi-Domain Adaptation with Spectral Consistency\n(Swin-T Backbone)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/1. sac_aw_d_accuracy_curve.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/1. sac_aw_d_accuracy_curve.pdf', bbox_inches='tight')
plt.show()
...
plt.title('Loss Components: Multi-Domain Adaptation SAC Training Dynamics\n(Spectral Adversarial Consistency)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.grid(True, alpha=0.3)
plt.yscale('log')  # Log scale to see all components clearly

plt.tight_layout()
plt.savefig(f'{plot_dir}/2. sac_aw_d_loss_components.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/2. sac_aw_d_loss_components.pdf', bbox_inches='tight')
plt.show()
...
plt.title('Multi-Domain Adaptation Confidence Threshold Schedule: Reverse Curriculum Learning', fontsize=16, fontweight='bold')
plt.grid(True, alpha=0.3)
plt.ylim(0.75, 1.0)

plt.tight_layout()
plt.savefig(f'{plot_dir}/3. sac_aw_d_confidence_schedule.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/3. sac_aw_d_confidence_schedule.pdf', bbox_inches='tight')
plt.show()
...
plt.title('Computational Efficiency: Multi-Domain Adaptation SAC Training Time\n(Swin-T + Spectral Consistency)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(f'{plot_dir}/4. sac_aw_d_timing_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/4. sac_aw_d_timing_analysis.pdf', bbox_inches='tight')
plt.show()
...
ax1.axhline(y=82.10, color='red', linestyle='--', linewidth=2, label='Baseline (82.10%)')
ax1.axhline(y=best_acc, color='green', linestyle='--', linewidth=2, label=f'Best SAC ({best_acc:.1f}%)')
ax1.set_ylabel('Accuracy (%)', fontsize=14, fontweight='bold')
ax1.set_title('A) SAC Performance: Multi-Domain Adaptation Results', fontsize=14, fontweight='bold')
...
ax2.set_title('B) Multi-Domain Adaptation Confidence Schedule: Reverse Curriculum', fontsize=14, fontweight='bold')
ax2.grid(True, alpha=0.3)
ax2.set_ylim(0.75, 1.0)

plt.tight_layout()
plt.savefig(f'{plot_dir}/5. sac_aw_d_combined_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/5. sac_aw_d_combined_analysis.pdf', bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/6. Multi-Domain adaptation sac_vs_baseline.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/6.Multi-Domain adaptation sac_vs_baseline.pdf', bbox_inches='tight')
plt.show()
...
print(f"Baseline Accuracy: 82.10%")
print(f"Improvement: {best_acc-82.10:+.2f}%")
...
summary = {
    ...
    'Baseline_Accuracy': 82.10,
    'Improvement': best_acc - 82.10,
...
summary_df.to_csv(f'{plot_dir}/sac_aw_d_performance_summary.csv', index=False)
print(f"\nAnalysis summary saved to: {plot_dir}/sac_aw_d_performance_summary.csv")

print(f"\n🎉 All plots saved to: {plot_dir}/")
print("✓ 1. sac_aw_d_accuracy_curve.png")
print("✓ 2. sac_aw_d_loss_components.png") 
print("✓ 3. sac_aw_d_confidence_schedule.png")
print("✓ 4. sac_aw_d_timing_analysis.png")
print("✓ 5. sac_aw_d_combined_analysis.png")
print("✓ 6. sac_aw_d_vs_baseline.png")
print("✓ sac_aw_d_performance_summary.csv")