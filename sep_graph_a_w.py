...
def create_individual_plots(csv_path):
    ...
    plt.axhline(y=94.58, color='r', linestyle='--', linewidth=2, label='Best Accuracy (94.34%)')
    plt.axvline(x=15, color='g', linestyle='--', linewidth=2, alpha=0.7, label='Best Epoch (14)')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('Test Accuracy Progress - Amazon → webcam Domain Adaptation', fontsize=14, fontweight='bold')
    plt.legend()
    ...
    plt.title('Cumulative Training Time\nTotal: 24.13 minutes', fontsize=12, fontweight='bold')
    plt.grid(True, alpha=0.3)
    ...
    plt.axhline(y=94.34, color='red', linestyle='--', linewidth=2, label='Best Accuracy (94.34%)')
    ...
    plt.figtext(0.1, 0.80, f'Amazon → webcam Adaptation: {current_best:.2f}% Accuracy', fontsize=14)
...
if __name__ == "__main__":
    csv_path = "./improved_results_amazon_to_webcam//amazon_to_webcam_swin_small_patch4_window7_224_improved_results_d_w_20250923_113433.csv"
    ...
    try:
        ...
        print(f"   Improvement from baseline: +{(df['Test_Accuracy'].max() - 78.06):.2f}%")
        print(f"   Relative improvement: +{((df['Test_Accuracy'].max() - 78.06) / 78.06 * 100):.1f}%")
