...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_impr1_results_d_w_{timestamp}.csv')
...
def save_checkpoint(model, optimizer, scheduler, history, epoch, test_acc, conf_threshold, output_dir, model_name):
    ...
    filename = os.path.join(output_dir, f'checkpoint_imp_d_W_epoch_{epoch+1}_{test_acc:.2f}.pth')
...
def main():
    ...
    output_dir = f'./improved1_results_{source_domain}_to_{target_domain}'
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_acc:
            ...
            # Save best model
            model_path = os.path.join(output_dir, f'best_improved_model_d_W_{model_name}.pth')
    ...
    if best_acc >= 98.88:
        ...
        print(f"↗️  IMPROVEMENT! Better than previous 98.88%")
    else:
