...
# Load your results
df = pd.read_csv('./results/swin_t office_11_ad_w_ssc_results.csv')
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/1. ssc_ad_w_accuracy_curve.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/1. ssc_ad_w_accuracy_curve.pdf', bbox_inches='tight')
plt.show()

# 2. Loss Components Analysis
plt.figure(figsize=(14, 8))
plt.plot(df['Epoch'], df['Train_Loss'], 'g-', linewidth=2.5, label='Classification Loss (Source)')
plt.plot(df['Epoch'], df['SSC_Loss'], 'b-', linewidth=2.5, label='SSC AD_W Loss (Target)')
...
plt.title('Loss Components: SSC Training Dynamics\n(AD → W Domain Adaptation)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/2. ssc_ad_w_loss_components.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/2. ssc_ad_w_loss_components.pdf', bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/3. ssc_ad_w_timing_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/3. ssc_ad_w_timing_analysis.pdf', bbox_inches='tight')
plt.show()
...
plt.title('Time-Accuracy Tradeoff: SSC Domain Adaptation\n(AD to W, Swin-T Backbone)', fontsize=16, fontweight='bold')
fig.tight_layout()
plt.savefig(f'{plot_dir}/4.ssc_ad_w_time_accuracy_tradeoff.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/4. ssc_ad_w_time_accuracy_tradeoff.pdf', bbox_inches='tight')
plt.show()
...
ax1.set_title('A) SSC Performance: AD → W Domain Adaptation', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/5.ssc_ad_w_combined_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/5. ssc_ad_w_combined_analysis.pdf', bbox_inches='tight')
plt.show()

# 6. Quantitative Analysis Report
print("=== SSC AD to W TRAINING ANALYSIS REPORT ===")
...
summary_df.to_csv(f'{plot_dir}/ssc_AD_to_W_performance_summary.csv', index=False)
print(f"\nAnalysis summary saved to: {plot_dir}/ssc_AD_to_W_performance_summary.csv")