...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_improved_results1__d_a_{timestamp}.csv')
...
def save_checkpoint(model, optimizer, scheduler, history, epoch, test_acc, conf_threshold, output_dir, model_name):
    ...
    filename = os.path.join(output_dir, f'checkpoint_epoch_d_a_{epoch+1}_{test_acc:.2f}.pth')
