...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_improved_results_1__w_d_{timestamp}.csv')
...
def save_checkpoint(model, optimizer, scheduler, history, epoch, test_acc, conf_threshold, output_dir, model_name):
    ...
    filename = os.path.join(output_dir, f'checkpoint_epoch_w_d_{epoch+1}_{test_acc:.2f}.pth')
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_acc:
            ...
            # Save best model
            model_path = os.path.join(output_dir, f'best_model1_w_d_{model_name}.pth')
    ...
    if best_acc >= 100.0:
        ...
        print(f"↗️  IMPROVEMENT! Better than previous 100.0%")
    else:
