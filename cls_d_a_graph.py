...
def analyze_experiment(csv_path):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title(f'{model_name} - D_A_Accuracy Curves\nBest Test: {best_accuracy:.2f}%')
    plt.legend()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'{model_name} - D_A_Loss Curves')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plot_path = os.path.join(plot_dir, f'{model_name}_d_a_training_curves.png')
    ...
    timing_path = os.path.join(plot_dir, f'{model_name}_d_a_timing_analysis.png')
...
if __name__ == '__main__':
    ...
    specific_csv_path = r"C:\\Users\\Admin\\Aruna_jrf_projects\\Datasets\\Office-31\\paper_experiments\\results\\swin_t_office_cls__d_a_results_20250917_125850.csv"
