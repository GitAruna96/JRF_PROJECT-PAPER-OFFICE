...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_mkmmd_d_a_results_{timestamp}.csv')
...
def main():
    ...
    src_train_loader = get_loader(name_dataset='dslr', batch_size=batch_size, train=True)
    trg_train_loader = get_loader(name_dataset='amazon', batch_size=batch_size, train=True)
    trg_evaluate_loader = get_loader(name_dataset='amazon', batch_size=batch_size, train=False)
    ...
    for model_name in model_names:
        ...
        for epoch in range(no_epochs):
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                counter = 0
                torch.save(model.state_dict(), f'MKmmd_d_a_{model_name}.pth')
