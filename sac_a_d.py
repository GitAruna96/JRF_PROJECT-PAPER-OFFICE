...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_sac_single_16_a_d_results_{timestamp}.csv')
...
def save_checkpoint(model, optimizer, history, epoch, test_acc, conf_threshold, output_dir, model_name):
    ...
    filename = os.path.join(output_dir, f'checkpoint_16_epoch_{epoch+1}_{test_acc:.2f}.pth')
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_acc:
            ...
            # Save best model
            model_path = os.path.join(output_dir, f'best_model_sac_16_a_d_{model_name}.pth')
        ...
        if (epoch + 1) % 5 == 0:
            save_checkpoint(model, optimizer, history, epoch, test_acc, current_conf, output_dir, f"{model_name}sac_16_a_d_epoch_{epoch+1}")
