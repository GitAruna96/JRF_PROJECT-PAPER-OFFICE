...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_improved_a_d_results_{timestamp}.csv')
...
def save_checkpoint(model, optimizer, scheduler, history, epoch, test_acc, conf_threshold, output_dir, model_name):
    ...
    filename = os.path.join(output_dir, f'checkpoint_imp_a_d__epoch_{epoch+1}_{test_acc:.2f}.pth')
...
def main():
    ...
    output_dir = f'./improved_a_d_results_{source_domain}_to_{target_domain}'
