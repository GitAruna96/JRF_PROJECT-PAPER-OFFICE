...
def create_individual_plots(csv_path):
    ...
    plt.axvline(x=15, color='g', linestyle='--', linewidth=2, alpha=0.7, label='Best Epoch (15)')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('Test Accuracy Progress - Amazon → DSLR Domain Adaptation', fontsize=14, fontweight='bold')
    plt.legend()
    ...
    plt.axhline(y=94.58, color='red', linestyle='--', linewidth=2, label='Best Accuracy (94.58%)')
    ...
    plt.figtext(0.1, 0.80, f'Amazon → DSLR Adaptation: {current_best:.2f}% Accuracy', fontsize=14)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"   Improvement from baseline: +{(df['Test_Accuracy'].max() - 73.10):.2f}%")
        print(f"   Relative improvement: +{((df['Test_Accuracy'].max() - 73.10) / 73.10 * 100):.1f}%")
