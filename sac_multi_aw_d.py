...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_aw_d_sac_predrev_results_{timestamp}.csv')
...
def save_checkpoint(model, optimizer, history, epoch, test_acc, conf_threshold, output_dir, model_name):
    ...
    filename = os.path.join(output_dir, f'checkpoint_aw_d_epoch_{epoch+1}_{test_acc:.2f}.pth')
...
def main():
    ...
    source_domains = ['amazon', 'webcam']  # Your multi-source domains
    target_domain = 'dslr'
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_acc:
            ...
            # Save regular model
            torch.save(model.state_dict(), os.path.join(output_dir, f'multi_source_sac_rev_pred_aw_d_{model_name}.pth'))
    ...
    # Save results
    save_history_to_csv(history, f'multi_source_{model_name}_Aw_d_sac_rev_pred', output_dir)
