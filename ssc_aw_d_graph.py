...
# Load your results
df = pd.read_csv('./results/swin_t office_11_aw_d_ssc_results.csv')

# Create output directory for plots
plot_dir = './results/plots/ssc_plots'
...
plt.plot(df['Epoch'], df['Test_Accuracy'], 'ro-', linewidth=2.5, markersize=8, label='Test Accuracy (Target - D)')
plt.plot(df['Epoch'], df['Train_Accuracy'], 'bo-', linewidth=2.5, markersize=8, label='Train Accuracy (Source - AW)', alpha=0.7)

plt.xlabel('Epoch', fontsize=14, fontweight='bold')
plt.ylabel('Accuracy (%)', fontsize=14, fontweight='bold')
plt.title('SSC Performance: AW → D Domain Adaptation\n(Swin-T Backbone, 50 Epochs)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/1. ssc_aw_d_accuracy_curve.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/1. ssc_aw_d_accuracy_curve.pdf', bbox_inches='tight')
plt.show()

# 2. Loss Components Analysis
plt.figure(figsize=(14, 8))
plt.plot(df['Epoch'], df['Train_Loss'], 'g-', linewidth=2.5, label='Classification Loss (Source)')
plt.plot(df['Epoch'], df['SSC_Loss'], 'b-', linewidth=2.5, label='SSC AW_D Loss (Target)')
...
plt.title('Loss Components: SSC Training Dynamics\n(AW → D Domain Adaptation)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/2. ssc_aw_d_loss_components.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/2. ssc_aw_d_loss_components.pdf', bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/3. ssc_aw_d_timing_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/3. ssc_aw_d_timing_analysis.pdf', bbox_inches='tight')
plt.show()
...
plt.title('Time-Accuracy Tradeoff: SSC Domain Adaptation\n(Aw to d, Swin-T Backbone)', fontsize=16, fontweight='bold')
fig.tight_layout()
plt.savefig(f'{plot_dir}/4.ssc_aw_d_time_accuracy_tradeoff.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/4. ssc_aw_d_time_accuracy_tradeoff.pdf', bbox_inches='tight')
plt.show()
...
# Top: Accuracy
ax1.plot(df['Epoch'], df['Test_Accuracy'], 'ro-', linewidth=2.5, markersize=6, label='Test Accuracy (D)')
ax1.plot(df['Epoch'], df['Train_Accuracy'], 'bo-', linewidth=2.5, markersize=6, label='Train Accuracy (AW)', alpha=0.7)
ax1.set_ylabel('Accuracy (%)', fontsize=14, fontweight='bold')
ax1.set_title('A) SSC Performance: AW to D Domain Adaptation', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/5.ssc_aw_d_combined_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/5. ssc_aw_d_combined_analysis.pdf', bbox_inches='tight')
plt.show()

# 6. Quantitative Analysis Report
print("=== SSC AW to D TRAINING ANALYSIS REPORT ===")
...
summary_df.to_csv(f'{plot_dir}/ssc_AW_to_D_performance_summary.csv', index=False)
print(f"\nAnalysis summary saved to: {plot_dir}/ssc_AW_to_D_performance_summary.csv")