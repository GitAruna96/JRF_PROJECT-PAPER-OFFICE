...
# Load your results
df = pd.read_csv('C:\\Users\\Admin\\Aruna_jrf_projects\\Datasets\\Office-31\\paper_experiments\\results\\swin_t_mkmmd_d_w_results_20250917_121857.csv')
...
plt.plot(df['Epoch'], df['Test_Accuracy'], 'ro-', linewidth=2.5, markersize=8, label='Test Accuracy (Target - WEBCAM)')
plt.plot(df['Epoch'], df['Train_Accuracy'], 'bo-', linewidth=2.5, markersize=8, label='Train Accuracy (Source - DSLR )', alpha=0.7)

plt.xlabel('Epoch', fontsize=14, fontweight='bold')
plt.ylabel('Accuracy (%)', fontsize=14, fontweight='bold')
plt.title('MKMMD Performance: D → W Domain Adaptation\n(Swin-T Backbone, 50 Epochs)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/1. mkmmd_d_w_accuracy_curve.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/1. mkmmd_d_w_accuracy_curve.pdf', bbox_inches='tight')
plt.show()
...
plt.title('Loss Components: MKMMD Training Dynamics\n(D → W Domain Adaptation)', fontsize=16, fontweight='bold')
plt.legend(fontsize=12)
plt.grid(True, alpha=0.3)
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/2. mkmmd_d_w_loss_components.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/2. mkmmd_d_w_loss_components.pdf', bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/3. mkmmd_d_w_timing_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/3. mkmmd_d_w_timing_analysis.pdf', bbox_inches='tight')
plt.show()
...
plt.title('Time-Accuracy Tradeoff: MKMMD Domain Adaptation\n(D → W, Swin-T Backbone)', fontsize=16, fontweight='bold')
fig.tight_layout()
plt.savefig(f'{plot_dir}/4.mkmmd_d_w_time_accuracy_tradeoff.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/4. mkmmd_d_w_time_accuracy_tradeoff.pdf', bbox_inches='tight')
plt.show()
...
# Top: Accuracy
ax1.plot(df['Epoch'], df['Test_Accuracy'], 'ro-', linewidth=2.5, markersize=6, label='Test Accuracy (W)')
ax1.plot(df['Epoch'], df['Train_Accuracy'], 'bo-', linewidth=2.5, markersize=6, label='Train Accuracy (D)', alpha=0.7)
ax1.set_ylabel('Accuracy (%)', fontsize=14, fontweight='bold')
ax1.set_title('A) MKMMD Performance: D → W Domain Adaptation', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig(f'{plot_dir}/5.mkmmd_d_w_combined_analysis.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{plot_dir}/5. mkmmd_d_w_combined_analysis.pdf', bbox_inches='tight')
plt.show()

# 6. Quantitative Analysis Report
print("=== MKMMD d_w_TRAINING ANALYSIS REPORT ===")
...
summary_df.to_csv(f'{plot_dir}/MKMMD_d_w_performance_summary.csv', index=False)
print(f"\nAnalysis summary saved to: {plot_dir}/MKMMD_d_w_performance_summary.csv")