...
def spectral_perturbation(x, noise_scale=0.05, high_freq_ratio=0.7):
    ...
    for i in range(x.shape[0]):
        ...
        x_perturbed_i = fft.ifft2(freq_back).real
        x_perturbed[i] = x_perturbed_i
...
def train_epoch(model, src_train_loader, trg_train_loader, optimizer, 
    ...
    # Use iterators to handle different dataset sizes
    src_iter = iter(src_train_loader)
    trg_iter = iter(trg_train_loader)
    
    for i in range(min(len(src_train_loader), len(trg_train_loader))):
        try:
...
def save_history_to_csv(history, model_name, output_dir):
    ...
    csv_path = os.path.join(output_dir, f'{model_name}_sac_single_d_a_results_{timestamp}.csv')
...
def save_checkpoint(model, optimizer, history, epoch, test_acc, conf_threshold, output_dir, model_name):
    ...
    filename = os.path.join(output_dir, f'checkpoint_d_a_epoch_{epoch+1}_{test_acc:.2f}.pth')
...
def main():
    ...
    num_classes = 31
    patience = 100  # Reduced early stopping patience
    ...
    src_train_loader, src_dataset = get_loader(source_domain, batch_size, train=True, domain='source')
    trg_train_loader, trg_dataset = get_loader(target_domain, batch_size, train=True, domain='target')
    trg_evaluate_loader, _ = get_loader(target_domain, batch_size, train=False, domain='target')

    # Verify number of classes
    actual_classes = len(src_dataset.classes)
    ...
    # Print dataset sizes
    print(f"Source ({source_domain}) samples: {len(src_dataset)}")
    print(f"Target train ({target_domain}) samples: {len(trg_dataset)}")
    ...
    for epoch in range(num_epochs):
        ...
        if test_acc > best_acc:
            ...
            # Save best model
            model_path = os.path.join(output_dir, f'sac_d_a_{model_name}.pth')
